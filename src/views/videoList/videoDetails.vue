...
export default class EnterpriseVideo extends Vue {
  ...
  private openVideo(_item){
    ...
    let sourceone=document.querySelector("#source");
    let myvideo_1=document.querySelector("#my_video_1");
    
    console.log(sourceone)
    console.log(myvideo_1)
    console.log("player",player)
    _this.$createDialog({
      type: 'alert',
      // icon: 'cubeic-alert',
      content:'<video id="my_video_1" class="video-js vjs-default-skin" controls preload="auto" style="width:100%" data-setup="{}"> <source id="source" src="" type="application/x-mpegURL"></video>',
      showClose: true,
      title: '',
      ...
      onClose: () => {
        ...
        // }).show()
        //player.paused()
        // console.log(document.querySelector(".cube-dialog"))
        // document.querySelector(".cube-dialog").remove()
        // console.log(document.querySelector(".cube-dialog"))
        player.dispose()
        clearInterval(_this.intervalint)
      }
    }).show()
    ...
    _this.videoRequest(_item.deviceCode,function(response){
      console.log("videoMonitor==>",response)
       if (response.data) {
              ...
              let url=obj.url;
              console.log("init one==>"+url)
              if(url.indexOf("http://172.16.7.2:83")>=0){
                url= url.replace("http://172.16.7.2:83","http://220.179.173.29:83")
              }
             console.log("init two==>"+url)
              let videoDoc=document.querySelector("#source")
              if(!videoDoc){
                console.log("没有对象了")
                let contentHtml=document.querySelector(".cube-dialog-content-def");
                console.log(contentHtml)
                console.log(contentHtml.querySelector("p"))
                contentHtml.querySelector("p").innerHTML='<video id="my_video_1" class="video-js vjs-default-skin" controls preload="auto" width="100%" height="50" data-setup="{}"> <source id="source" src="" type="application/x-mpegURL"></video>';
                videoDoc=document.querySelector("#source")
              }
              videoDoc['src']=url;
    ...
    _this.intervalint = setInterval(()=>{
      _this.videoRequest(_item.deviceCode,function(response){
        console.log("videoMonitor= Interval =>",response)
        if (response.data) {
              console.log(response.data)
              let obj=response.data.data;
              let url=obj.url;
